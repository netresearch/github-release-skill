---
name: github-release
description: "Use when creating releases, version bumps, tagging, release health checks, or when user says 'release', 'tag', 'version bump'. A version lives in every file that states it, not only the obvious one — a TYPO3 extension carries it in ext_emconf.php, composer.json, Documentation/guides.xml, the rendered changelog page and CHANGELOG.md, and a release that updates some of them ships metadata that disagrees with itself. Also activates on gh release commands to BLOCK them and redirect to safe process."
